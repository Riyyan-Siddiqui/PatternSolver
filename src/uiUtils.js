...
import Background from "../public/bg6.jpeg";
...
// Allow arrow keys to slide puzzle tiles during play mode
document.addEventListener("keydown", (event) => {
	if (!state.playMode || state.solveAnimation.active) return;
	...
	if (moved) {
		updatePuzzleState(puzzle.matrix);
		disableGridTileDragging();
		playModeSetMovableTiles();
	}
});
